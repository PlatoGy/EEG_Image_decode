...
from DC_dynamic_chunk_router import DynamicChunkATMS, make_dynamic_chunk_router_modules
...
def load_dynamic_models(args, device):
    state = torch.load(args.diffusion_prior_ckpt, map_location=device)
    if "diffusion_prior" not in state:
        raise ValueError("dynamic_chunk_router checkpoint must contain diffusion_prior, router, gamma, and boundary_predictor.")

    dynamic_config = state.get("dynamic_config", {})
    min_chunk_length = dynamic_config.get("min_chunk_length", args.min_chunk_length)
    mask_temperature = dynamic_config.get("mask_temperature", args.mask_temperature)
    boundary_hidden_dim = dynamic_config.get("boundary_hidden_dim", args.boundary_hidden_dim)
    ...
    pipe = make_dynamic_chunk_router_modules(device, cond_dim=1024, dropout=args.dropout, init_gamma=args.init_gamma)
    ...
    print("loaded dynamic chunk router checkpoint:", args.diffusion_prior_ckpt)
    print("gamma:", float(pipe.router_condition.gamma.detach().cpu().item()))
    print("dynamic_config:", dynamic_config)
    return conditioner, pipe


@torch.no_grad()
def extract_dynamic_condition_cache(args, device, run_dir, conditioner):
    if not args.atms_ckpt:
        raise ValueError("dynamic_chunk_router mode requires --atms-ckpt.")
    ...
    cache_path = run_dir / f"dynamic_chunk_condition_cache_{args.subject}.pt"
    torch.save(condition_cache, cache_path)
    print("saved dynamic condition cache:", cache_path)
    ...
    stats_path = run_dir / f"dynamic_boundaries_{args.subject}.csv"
    with open(stats_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["sample_index", "b1", "b2", "b3", "len1", "len2", "len3", "len4"])
        for idx in range(condition_cache["boundaries"].shape[0]):
            writer.writerow([
                idx,
                *[float(x) for x in condition_cache["boundaries"][idx].tolist()],
                *[float(x) for x in condition_cache["lengths"][idx].tolist()],
            ])
    print("saved dynamic boundary stats:", stats_path)
    return condition_cache


def parse_args():
    parser = argparse.ArgumentParser(description="Generate images with Dynamic 4-Chunk + Timestep Router prior.")
...
def main():
    ...
    conditioner, pipe = load_dynamic_models(args, device)
    condition_cache = extract_dynamic_condition_cache(args, device, run_dir, conditioner)
