...
from DC_dynamic_chunk_router import (
    ...
    make_dynamic_optimizer_and_scheduler,
)
...
def validate_dynamic_inputs(train_eeg, raw_img_train, repeated_img_train):
    ...
    print("dynamic check: train EEG", tuple(train_eeg.shape))
    print("dynamic check: raw image targets", tuple(raw_img_train.shape))
    print("dynamic check: repeated image targets", tuple(repeated_img_train.shape))
    print("dynamic check: repeat max_abs_diff", max_repeat_diff)
    print(
        "dynamic check: raw target norm",
...
@torch.no_grad()
def dynamic_prior_retrieval_metrics(pipe, conditioner, eeg, img_features, subject_id, device, args):
...
def save_dynamic_checkpoint(pipe, conditioner, path, mode, args, epoch=None):
    ...
    torch.save(
        {
            ...
            "boundary_predictor": conditioner.boundary_predictor.state_dict(),
            "dynamic_config": {
...
def load_resume_checkpoint(pipe, conditioner, checkpoint_path, device):
    state = torch.load(checkpoint_path, map_location=device)
    if "diffusion_prior" not in state:
        raise ValueError("Dynamic resume checkpoint must contain diffusion_prior, router, gamma, and boundary_predictor.")
    ...
    conditioner.boundary_predictor.load_state_dict(state["boundary_predictor"])
    print("loaded dynamic checkpoint:", checkpoint_path)


def train_dynamic_chunk_router(args, run_dir, device):
    if not args.atms_ckpt:
        raise ValueError("--atms-ckpt is required for dynamic_chunk_router mode.")
    ...
    validate_dynamic_inputs(train_eeg, raw_img_train, target_img_train)
    ...
    pipe = make_dynamic_chunk_router_modules(
        device,
    ...
    boundary_trainable, boundary_total = count_trainable(conditioner.boundary_predictor)
    ...
    for epoch_idx in range(args.epochs):
        ...
        if should_save:
            save_dynamic_checkpoint(
                pipe,
                conditioner,
                run_dir / "checkpoints" / f"epoch_{epoch_idx + 1:03d}.pth",
                args.mode,
                args,
                epoch_idx + 1,
            )
            save_dynamic_checkpoint(pipe, conditioner, run_dir / "latest.pth", args.mode, args, epoch_idx + 1)

        if args.eval_every > 0 and ((epoch_idx + 1) % args.eval_every == 0 or epoch_idx + 1 == args.epochs):
            cosine, top1 = dynamic_prior_retrieval_metrics(pipe, conditioner, test_eeg, test_img, subject_id, device, args)
    ...
    save_dynamic_checkpoint(pipe, conditioner, run_dir / args.save_name, args.mode, args, args.epochs)
    print("saved dynamic diffusion prior:", run_dir / args.save_name)


def parse_args():
    parser = argparse.ArgumentParser(description="Train Dynamic 4-Chunk + Timestep Router diffusion prior.")
    parser.add_argument("--mode", choices=["dynamic_chunk_router"], default="dynamic_chunk_router")
...
def main():
    ...
    print("init chunks:", DYNAMIC_INIT_CHUNKS)
    print("run_dir:", run_dir)
    train_dynamic_chunk_router(args, run_dir, device)
    print("run_dir:", run_dir)
