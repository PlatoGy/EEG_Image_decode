...
def validate_inputs(train_eeg, raw_img_train, repeated_img_train):
    ...
    print("featboundary check: train EEG", tuple(train_eeg.shape))
    print("featboundary check: raw image targets", tuple(raw_img_train.shape))
    print("featboundary check: repeated image targets", tuple(repeated_img_train.shape))
    print("featboundary check: repeat max_abs_diff", max_repeat_diff)
    print(
        "featboundary check: raw target norm",
...
def load_resume_checkpoint(pipe, checkpoint_path, device):
...
def train_featboundary_router(args, run_dir, device):
    ...
    print("saved feature-boundary diffusion prior:", run_dir / args.save_name)


def parse_args():
    parser = argparse.ArgumentParser(description="Train Feature-Change Boundary 4-Chunk + Timestep Router diffusion prior.")
    parser.add_argument("--mode", choices=["featboundary_router"], default="featboundary_router")
...
def main():
    ...
    train_featboundary_router(args, run_dir, device)
    print("run_dir:", run_dir)
